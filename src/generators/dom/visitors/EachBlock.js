...
import { getUniqueLocalAliasMaker } from '../../shared/utils/aliaser.js';
...
export default {
	enter ( generator, node ) {
		...
		const listName = generator.getUniqueName( `${name}_value` );
		...
		if ( node.key ) {
			...
			initialRender.addBlock( deindent`
				...
				${localVars.iterations}[${i}] = ${localVars.lookup}[ ${localVars.key} ] = ${renderer}( ${params}, ${listName}, ${listName}[${i}], ${i}, component${node.key ? `, ${localVars.key}` : `` } );
			` );
		} else {
			initialRender.addLine(
				`${localVars.iterations}[${i}] = ${renderer}( ${params}, ${listName}, ${listName}[${i}], ${i}, component );`
			);
		}
			...
			generator.current.builders.update.addBlock( deindent`
				...
				for ( var ${i} = 0; ${i} < ${listName}.length; ${i} += 1 ) {
					...
					if ( ${localVars.lookup}[ ${localVars.key} ] ) {
						...
						${localVars._iterations}[${i}] = ${localVars._lookup}[ ${localVars.key} ] = ${renderer}( ${params}, ${listName}, ${listName}[${i}], ${i}, component${node.key ? `, ${localVars.key}` : `` } );
					}
		...
		const indexName = node.index || generator.getUniqueName( `${node.context}__index` );
		...
		generator.push({
			...
			getUniqueName: getUniqueLocalAliasMaker( blockParams )
		});
	},
